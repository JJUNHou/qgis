...
shapefile_path1_1 = "C:/Users/dlwns/Downloads/distribution (1)/korea_province/korea_province(시도).shp" # Shapefile 경로 지정
layer1_1 = QgsVectorLayer(shapefile_path1_1, "MyLayer1_1", "ogr") # 레이어 불러오기

# 레이어가 성공적으로 불러와지면 레이어를 맵에 추가합니다.
if layer1_1.isValid():
    QgsProject.instance().addMapLayer(layer1_1)
    print("레이어가 성공적으로 불러와졌습니다.")
else:
    print("레이어를 불러오는데 문제가 발생했습니다.")

#스타일 불러오기
existing_layer_name = "MyLayer1_1"
existing_layers = QgsProject.instance().mapLayersByName(existing_layer_name)
if existing_layers:
    existing_layer = existing_layers[0]  # 첫 번째 레이어를 선택
    print(f"{existing_layer_name} 레이어가 활성화되었습니다.")

style_path = 'D:/coding/stone_distribution_1/style/MyLayer1_1.qml'
existing_layer.loadNamedStyle(style_path)
existing_layer.triggerRepaint()
...
shapefile_path2 = "C:/Users/dlwns/Downloads/distribution (1)/shp/거창석.shp" # Shapefile 경로를 지정합니다.
